...
angular.module('tm.pagination', []).directive('tmPagination',[function(){
    return {
        ...
        template: '<div class="page">' +
        	...
        	'</ul>' +
            '<ul ng-show="conf.totalItems > 0">' + 
            '<li  class="pre_page"><a href="javascript:void(0)" ng-class="{disabled: conf.currentPage == 1}" ng-click="prevPage()"><</a></li>' +
            '<li ng-repeat="item in pageList track by $index" ng-class="{index_page: item == conf.currentPage, separate: item == \'...\'}" ' +
            'ng-click="changeCurrentPage(item)">' +
            '<a href="javascript:void(0);">{{ item }}</a>' +
            '</li>' +
            '<li class="next_page" ng-click="nextPage()"><a href="javascript:void(0)"  ng-class="{disabled: conf.currentPage == conf.numberOfPages}">></a></li>' +
            '<li ng-show="conf.totalItems > 0">' + 
            '</li>' + 
            '<li ng-show="conf.totalItems > 0">' + 
            '<span>跳转到：</span>' + 
            '</li>' + 
            '<li ng-show="conf.totalItems > 0">' + 
            '<input type="text" ng-model="jumpPageNum"/>' + 
            '</li>' + 
            '<li ng-show="conf.totalItems > 0">' + 
            '<span>页</span>' + 
            '</li>' + 
            '<li><input type="submit" name="go" value="GO" ng-click="jumpToPage($event)"></li>' +
            '</ul>',
        replace: true,
        scope: {
            conf: '='
